...
1. **x64 builds:** The patcher scans for the unique gate signature: `test rax,rax` → `je` (eligible) → `cmp byte ptr [rax+8],0` → `jne` (eligible) → `call failure_builder` → spills of `rax`, `rbx`, and `rcx` to `[rsp+0x80]`, `[rsp+0x50]`, and `[rsp+0x70]`. An additional supported x64 layout, observed in newer builds (1.2.5+), spills `rax`, `rbx`, and `rcx` to `[rsp+0x88]`, `[rsp+0x50]`, and `[rsp+0x78]`; a separate signature constrains those exact instructions.
